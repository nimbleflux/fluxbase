...
const POSTGRESQL_FUNCTIONS = [
  ...
  {
    name: 'COUNT',
    detail: 'count(*) or count(expression)',
    insertText: 'COUNT(${1:*})',
  },
  ...
  {
    name: 'ARRAY_AGG',
    detail: 'array_agg(expression)',
    insertText: 'ARRAY_AGG(${1:column})',
  },
  {
    name: 'STRING_AGG',
    detail: 'string_agg(expression, delimiter)',
    insertText: "STRING_AGG(${1:column}, ${2:', '})",
  },
  {
    name: 'JSON_AGG',
    detail: 'json_agg(expression)',
    insertText: 'JSON_AGG(${1:column})',
  },
  {
    name: 'JSONB_AGG',
    detail: 'jsonb_agg(expression)',
  ...
  {
    name: 'CONCAT_WS',
    detail: 'concat_ws(separator, str1, str2, ...)',
  ...
  { name: 'RTRIM', detail: 'rtrim(string)', insertText: 'RTRIM(${1:string})' },
  {
    ...
    insertText: 'SUBSTRING(${1:string}, ${2:1}, ${3:10})',
  },
  {
    name: 'REPLACE',
    detail: 'replace(string, from, to)',
    insertText: 'REPLACE(${1:string}, ${2:from}, ${3:to})',
  },
  {
    name: 'SPLIT_PART',
    detail: 'split_part(string, delimiter, position)',
    insertText: "SPLIT_PART(${1:string}, ${2:','}, ${3:1})",
  },
  {
    name: 'REGEXP_REPLACE',
    detail: 'regexp_replace(string, pattern, replacement)',
    insertText: 'REGEXP_REPLACE(${1:string}, ${2:pattern}, ${3:replacement})',
  },
  ...
  {
    name: 'DATE_TRUNC',
    detail: 'date_trunc(field, source)',
    insertText: "DATE_TRUNC(${1:'day'}, ${2:timestamp})",
  },
  {
    name: 'DATE_PART',
    detail: 'date_part(field, source)',
  ...
  {
    name: 'AGE',
    detail: 'age(timestamp1, timestamp2)',
    insertText: 'AGE(${1:timestamp1}, ${2:timestamp2})',
  },
  {
    name: 'TO_CHAR',
    detail: 'to_char(timestamp, format)',
    insertText: "TO_CHAR(${1:timestamp}, ${2:'YYYY-MM-DD'})",
  },
  {
    name: 'TO_DATE',
    detail: 'to_date(string, format)',
    insertText: "TO_DATE(${1:string}, ${2:'YYYY-MM-DD'})",
  },
  {
    name: 'TO_TIMESTAMP',
    detail: 'to_timestamp(string, format)',
  ...
  // JSON functions
  {
    ...
    detail: 'json_build_object(key1, val1, ...)',
  ...
  },
  {
    ...
    detail: 'json_array_length(json)',
    insertText: 'JSON_ARRAY_LENGTH(${1:json_column})',
  },
  {
    name: 'JSONB_ARRAY_LENGTH',
    detail: 'jsonb_array_length(jsonb)',
    insertText: 'JSONB_ARRAY_LENGTH(${1:jsonb_column})',
  },
  {
    name: 'TO_JSON',
    detail: 'to_json(value)',
    insertText: 'TO_JSON(${1:value})',
  },
  {
    name: 'TO_JSONB',
    detail: 'to_jsonb(value)',
  ...
  // UUID
  {
    ...
    detail: 'gen_random_uuid() - generate UUID',
...
function getCompletionContext(
  ...
  const beforePrefix = textBeforeCursor
    .substring(0, textBeforeCursor.length - prefix.length)
    .trim()
  ...
  return {
    ...
    provideCompletionItems(
      ...
      const range: Monaco.IRange = {
        ...
        startColumn: hasDot
          ? position.column - afterDot.length
          : word.startColumn,
      ...
      if (hasDot && dotParts.length === 2) {
        ...
        const isSchema = metadata.schemas.some(
          (s) => s.toLowerCase() === qualifier
        )

        if (isSchema) {
          // Suggest tables in this schema
          const tablesInSchema = metadata.tables.filter(
            (t) => t.schema.toLowerCase() === qualifier
          )
      ...
      if (tableContextKeywords.includes(wordBefore)) {
        ...
        for (const [alias, table] of tableAliases) {
          for (const column of table.columns) {
            ...
            const columnLabel =
              tableAliases.size > 1 ? `${alias}.${column.name}` : column.name
