...
const KEY_TYPES = [
  ...
  { value: 'publishable', label: 'Publishable', description: 'Client-side SDK access' },
  { value: 'tenant_service', label: 'Tenant Service', description: 'Server-side tenant access' },
  { value: 'global_service', label: 'Global Service', description: 'Admin-level system access' },
] as const
...
export function CreateKeyDialog({ open, onOpenChange, onSuccess }: CreateKeyDialogProps) {
  ...
  const [selectedKeyType, setSelectedKeyType] = useState<KeyType>('publishable')
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateKeyFormData) => {
      return platformServiceKeysApi.create({
        ...
        tenant_id: data.key_type === 'tenant_service' ? data.tenant_id : undefined,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new service key for API access. The key will be shown only once.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {selectedKeyType === 'tenant_service' && (
              <div className="grid gap-2">
                ...
                {tenantsLoading && (
                  <p className="text-muted-foreground text-xs">Loading tenants...</p>
                )}
              </div>
            )}
