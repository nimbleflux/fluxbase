...
test.describe("Tenant CRUD", () => {
  ...
  test("cannot delete default tenant", async ({ adminPage }) => {
    // Navigate to tenants page
    await adminPage.goto("tenants", { waitUntil: "networkidle" });
    await expect(adminPage.getByText("Default")).toBeVisible({
      timeout: 10_000,
    });

    // Find the default tenant row
    const defaultRow = adminPage
      .getByRole("row")
      .filter({ hasText: "Default" });

    // Default tenant should NOT have a delete button
  ...
  test("tenant list shows correct count", async ({ adminPage, adminToken }) => {
    ...
    // Check the "Total Tenants" card
    const totalCard = adminPage.getByText("Total Tenants").locator("..");
    const totalText = await totalCard.textContent();
    const total = parseInt(totalText?.match(/\d+/)?.[0] || "0", 10);
    expect(total).toBeGreaterThanOrEqual(3); // default + 2 new
