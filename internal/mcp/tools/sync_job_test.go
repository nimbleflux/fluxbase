...
func TestJobAnnotations_Struct(t *testing.T) {
	t.Run("zero value", func(t *testing.T) {
		var config jobs.JobAnnotations
...
func TestParseJobAnnotations(t *testing.T) {
	t.Run("empty code returns defaults", func(t *testing.T) {
		config := jobs.ParseJobAnnotations("")
...
export default async function cleanup() {
  // Cleanup logic
		...
		assert.Equal(t, 1024, config.MemoryLimitMB)
		assert.Equal(t, 5, config.MaxRetries)
		assert.True(t, config.DisableExecutionLogs)
	})
