...
func (s *Storage) UpdateMigration(ctx context.Context, namespace, name string, updates map[string]interface{}) error {
	...
	query := "UPDATE platform.migrations SET updated_at = NOW()"
	args := []interface{}{}
	argCount := 1
...
func (s *Storage) DeleteMigration(ctx context.Context, namespace, name string) error {
	query := "DELETE FROM platform.migrations WHERE namespace = $1 AND name = $2 AND status = 'pending'"
...
func (s *Storage) UpdateMigrationStatus(ctx context.Context, id uuid.UUID, status string, appliedBy *uuid.UUID) error {
	...
	switch status {
	case "applied":
		query = "UPDATE platform.migrations SET status = $1, applied_at = NOW(), applied_by = $2, updated_at = NOW() WHERE id = $3"
		args = []interface{}{status, appliedBy, id}
	case "rolled_back":
		query = "UPDATE platform.migrations SET status = $1, rolled_back_at = NOW(), updated_at = NOW() WHERE id = $2"
		args = []interface{}{status, id}
	case "failed":
		query = "UPDATE platform.migrations SET status = $1, updated_at = NOW() WHERE id = $2"
		args = []interface{}{status, id}
	default:
...
func (s *Storage) LogExecution(ctx context.Context, log *ExecutionLog) error {
	query := `
		INSERT INTO platform.migration_execution_logs (
...
func (s *Storage) GetExecutionLogs(ctx context.Context, migrationID uuid.UUID, limit int) ([]ExecutionLog, error) {
	query := `
		...
		FROM platform.migration_execution_logs
